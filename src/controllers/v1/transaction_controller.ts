    ...
    try {
        ...
        for (const account of bankAccounts) {
            ...
            while (hasMore) {
                ...
                await storeSyncTransactions(added, modified, removed);
...
async function storeGetTransactions(transactions: any[]): Promise<void> {
...
async function storeSyncTransactions(added: any[], modified: any[], removed: any[]): Promise<void> {
    ...
    for (const trans of modified) {
        await Transaction.update(mapTransactionFields(trans), {
            where: { transaction_id: trans.transaction_id }
        });
    }
}

function mapTransactionFields(trans: any) {
    return {
